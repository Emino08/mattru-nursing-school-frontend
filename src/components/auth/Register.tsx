...
export default function Register() {
    ...
    const generateCaptcha = () => {
        ...
        const num2 = Math.floor(Math.random() * 10) + 1;
        setCaptcha({ num1, num2, answer: num1 + num2 });
        reset({ captchaAnswer: '' }, { keepValues: true });
    };

    const onSubmit = async (data: FormData) => {
        try {
            // Verify the captcha locally first
            if (parseInt(data.captchaAnswer) !== captcha.answer) {
                toast.error('Incorrect captcha answer. Please try again.');
                generateCaptcha();
                return;
            ...
            await api.post('/register', {
                email: data.email,
                password: data.password,
                captcha: 'valid_captcha', // Send the hardcoded value the backend expects
                profile: {
                    ...
                    emergency_contact: {
                        name: data.profile.emergency_contact.name,
                        phone: data.profile.emergency_contact.phone
                    },
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <div>
                                ...
                                <Input
                                    ...
                                    style={{ borderRadius: '0.375rem' }}
                                />
                                {errors.profile?.phone && <p className="text-red-500 text-sm mt-1">{errors.profile.phone.message}</p>}
                            </div>

                            <div className="border-t border-gray-200 pt-4">
                                <h3 className="font-medium text-gray-800 mb-4">Address Information</h3>
                                <div className="grid md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            style={{ borderRadius: '0.375rem' }}
                                        />
                                        {errors.profile?.address?.street && <p className="text-red-500 text-sm mt-1">{errors.profile.address.street.message}</p>}
                                    </div>
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            style={{ borderRadius: '0.375rem' }}
                                        />
                                        {errors.profile?.address?.city && <p className="text-red-500 text-sm mt-1">{errors.profile.address.city.message}</p>}
                                    </div>
                                    <div>
                                        <Label className="text-gray-700">District</Label>
                                        <Input
                                            {...register('profile.address.district', { required: 'District is required' })}
                                            placeholder="Western Area"
                                            className="border-gray-200"
                                            style={{ borderRadius: '0.375rem' }}
                                        />
                                        {errors.profile?.address?.district && <p className="text-red-500 text-sm mt-1">{errors.profile.address.district.message}</p>}
                                    </div>
                                    <div>
                                        <Label className="text-gray-700">Country</Label>
                                        <Input
                                            {...register('profile.address.country', { required: 'Country is required' })}
                                            placeholder="Sierra Leone"
                                            className="border-gray-200"
                                            style={{ borderRadius: '0.375rem' }}
                                        />
                                        {errors.profile?.address?.country && <p className="text-red-500 text-sm mt-1">{errors.profile.address.country.message}</p>}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <Input
                                        {...register('profile.date_of_birth', { required: 'Date of birth is required' })}
                                        type="date"
                                        className="border-gray-200"
                                        style={{ borderRadius: '0.375rem' }}
                                    />
                                    {errors.profile?.date_of_birth && <p className="text-red-500 text-sm mt-1">{errors.profile.date_of_birth.message}</p>}
                                </div>
                                <div>
                                    <Label className="text-gray-700">Nationality</Label>
                                    <Input
                                        {...register('profile.nationality', { required: 'Nationality is required' })}
                                        placeholder="Sierra Leonean"
                                        className="border-gray-200"
                                        style={{ borderRadius: '0.375rem' }}
                                    />
                                    {errors.profile?.nationality && <p className="text-red-500 text-sm mt-1">{errors.profile.nationality.message}</p>}
                                </div>
                            </div>

                            <div className="border-t border-gray-200 pt-4">
                                <h3 className="font-medium text-gray-800 mb-4">Emergency Contact</h3>
                                <div className="grid md:grid-cols-2 gap-4">
                                    <div>
                                        <Label className="text-gray-700">Name</Label>
                                        <Input
                                            {...register('profile.emergency_contact.name', { required: 'Emergency contact name is required' })}
                                            placeholder="Abu Koroma"
                                            className="border-gray-200"
                                            style={{ borderRadius: '0.375rem' }}
                                        />
                                        {errors.profile?.emergency_contact?.name && <p className="text-red-500 text-sm mt-1">{errors.profile.emergency_contact.name.message}</p>}
                                    </div>
                                    <div>
                                        <Label className="text-gray-700">Phone</Label>
                                        <Input
                                            {...register('profile.emergency_contact.phone', { required: 'Emergency contact phone is required' })}
                                            placeholder="078618435"
                                            className="border-gray-200"
                                            style={{ borderRadius: '0.375rem' }}
                                        />
                                        {errors.profile?.emergency_contact?.phone && <p className="text-red-500 text-sm mt-1">{errors.profile.emergency_contact.phone.message}</p>}
                                    </div>
                                </div>
                            </div>
